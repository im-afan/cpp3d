...
int main(int argc, char* argv[]){
	...
	std::vector<Eigen::Matrix4d> faceTransforms = {
		rotation3d(Eigen::Vector4d(0, 0, 0, 0)) * translation3d(Eigen::Vector4d(1, 0, 0, 0)),
		rotation3d(Eigen::Vector4d(0, 0, 0, 0)) * translation3d(Eigen::Vector4d(-1, 0, 0, 0)),
	};
	...
	while(true){
		...
		for(int j = 0; j < faceTransforms.size(); j++){
			for(int i = 0; i < inds.size(); i += 3){ //draw each triangle
				...
				Eigen::Matrix4d t = faceTransforms[j]  * transform;
